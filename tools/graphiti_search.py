...
def _format_group_id(template: str, user: dict) -> Optional[str]:
    """Format a group_id template with user data. Returns None if template is invalid or user_id is missing."""
    if not user or not isinstance(user, dict):
        return None  # Fail-close: don't allow group_id without valid user
    user_id = user.get('id')
    if not user_id:
        return None  # Fail-close: require user_id for group isolation
    ...
    try:
        ...
        # Invalid template (unknown placeholder or malformed braces)
        return None
...
class GraphitiHelper:
    ...
    def get_allowed_group_ids_with_desc(
        ...
        Returns:
            Tuple of (allowed_group_ids, descriptions, allow_none).
            allow_none is True if 'none' is in group_id_formats (disables group filtering).
        """
        ...
        return deduped_ids, descriptions, allow_none
    ...
    def get_allowed_group_ids(
        ...
        Returns:
            Tuple of (allowed_group_ids, allow_none).
            allow_none is True if 'none' is in group_id_formats.
        """
        allowed_group_ids, _, allow_none = self.get_allowed_group_ids_with_desc(user)
        return allowed_group_ids, allow_none

    def get_default_group_id(self, user: dict) -> tuple[Optional[str], bool]:
        """
        Get the default group_id (first entry in group_id_formats, or None if 'none' is first).

        Args:
            user: User dictionary containing 'id', 'email', 'name'

        Returns:
            Tuple of (default_group_id, allow_none).
            If 'none' is the first entry in group_id_formats, default_group_id is None.
        """
        # Check if 'none' is the first entry - if so, default to no filtering
        formats = _parse_group_id_formats(self.valves.group_id_formats)
        if formats and formats[0][0].lower() == 'none':
            return None, True

        allowed_group_ids, allow_none = self.get_allowed_group_ids(user)
        if allowed_group_ids:
            return allowed_group_ids[0], allow_none
    ...
    def get_group_id_selection(
        ...
        Args:
            user: User dictionary containing 'id', 'email', 'name'
            group_id: Optional group_id override. None uses the first entry in group_id_formats.
                      If 'none' is in group_id_formats, None is allowed (disables filtering).

        Returns:
            Tuple of (group_id, error_message). error_message is None if valid.
            group_id may be None if filtering is disabled via 'none' in group_id_formats.
        """
        allowed_group_ids, allow_none = self.get_allowed_group_ids(user)

        if group_id is None:
            default_group_id, _ = self.get_default_group_id(user)
            if default_group_id:
                return default_group_id, None
            # No default group_id: check if 'none' allows unrestricted access
            if allow_none:
                return None, None  # group filtering disabled
            return None, "❌ Error: Group ID is required. Please configure group_id_formats."

        # Runtime validation: AI may pass non-string values despite type annotation
        if not isinstance(group_id, str):  # type: ignore[unreachable]
            return None, (
                f"❌ Error: group_id must be a string or null. "
                f"Received type: {type(group_id).__name__}. "
                f"Allowed group_ids: {allowed_group_ids}"
            )

        normalized_group_id = _normalize_group_id(group_id)
        sanitized_group_id = _sanitize_group_id(normalized_group_id)

        # "none" or empty string means explicitly request no filtering (if allowed)
        if sanitized_group_id == '' or sanitized_group_id.lower() == 'none':
            if allow_none:
                return None, None
            return None, (
                "❌ Error: 'none' (no group filtering) is not allowed. "
                f"Allowed group_ids: {allowed_group_ids}"
            )

        if sanitized_group_id not in allowed_group_ids:
            return None, (
                f"❌ Error: group_id '{sanitized_group_id}' is not allowed. "
                f"Allowed group_ids: {allowed_group_ids}"
            )
...
class Tools:
    class Valves(BaseModel):
        ...
        group_id_formats: str = Field(
            default="{user_id}:Personal memory",
            description=(
                "Comma-separated group_id formats or fixed IDs. "
                "The FIRST entry is the default when group_id is not specified. "
                ...
                "Example: '{user_id}:Personal,common:Shared knowledge base'"
            ),
        )
    ...
    async def get_available_group_ids(
        ...
        - Know the default group_id (first entry, used when group_id is not specified)

        :return: JSON with resolved group_ids the AI should use, with descriptions embedded
        """
        try:
            __user__ = __user__ or {}
            allowed_group_ids, descriptions, allow_none = self.helper.get_allowed_group_ids_with_desc(__user__)
            default_group_id, _ = self.helper.get_default_group_id(__user__)
            ...
            # Add 'none' indicator if unrestricted access is allowed
            if allow_none:
                allowed_group_ids_with_desc.append("none:No group filtering (search all)")
            ...
            return json.dumps({"error": f"Failed to get available group_ids: {e}"})
    ...
    async def search_entities(
        ...
        # Always set (even empty) to clear any stale headers from previous requests
        headers = get_user_info_headers(self.valves, __user__, None)
        user_headers_context.set(headers)
        if headers and self.valves.debug_print:
            print(f"Set user headers in context: {list(headers.keys())}")

        try:
            # Validate and clamp limit (inside try for type safety)
    ...
    async def get_recent_episodes(
        ...
        try:
            # Validate and clamp parameters (inside try for type safety)
            limit = max(1, min(100, int(limit)))
    ...
    async def get_episode_content(
        ...
        try:
            ...
            # Verify the episode belongs to an allowed group_id
            episode_group_id = getattr(episode, 'group_id', None)
            allowed_group_ids, allow_none = self.helper.get_allowed_group_ids(__user__)

            # If allow_none is True, filtering is disabled - allow access to any episode
            if not allow_none:
                if episode_group_id:
                    if episode_group_id not in allowed_group_ids:
                        return "❌ Access denied: Episode does not belong to your allowed groups."
                else:
                    # Episode has no group_id - deny if 'none' is not configured
                    return "❌ Access denied: Episode has no group_id and 'none' is not allowed."
